...
#     star_players_score, roster_remaining = sum(impacts[:2]), impacts[2:]
#     if roster_remaining:
#         depth_score = sum(roster_remaining) / len(roster_remaining)
#     else:
#         depth_score = 0
    
#     team_strength = 0.7 * star_players_score + 0.3 * depth_score
#     teams.append({
...
for team, roster in current.groupby('TEAM_ABBREVIATION'):
    ...
    star_players_score, roster_remaining = sum(impacts[:2]) / 2, impacts[2:]
    if roster_remaining:
        depth_score = sum(roster_remaining) / len(roster_remaining)
    else:
        depth_score = 0
